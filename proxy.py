...
def build_driver():
    ...
    # Set Chrome binary if found (selenium-manager handles chromedriver automatically)
    for chrome in ["/usr/bin/google-chrome", "/usr/bin/chromium-browser", "/usr/bin/chromium"]:
        if os.path.exists(chrome):
            opts.binary_location = chrome
            print(f"  Usando Chrome: {chrome}")
            break
    # selenium 4.x selenium-manager downloads matching chromedriver automatically
