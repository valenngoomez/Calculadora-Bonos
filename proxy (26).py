...
def build_driver():
    ...
    opts = Options()
    # Headless solo si no hay display Xvfb disponible
    import subprocess as _sp
    has_display = os.environ.get("DISPLAY", "") != "" and _sp.run(["xdpyinfo"], capture_output=True).returncode == 0
    if not has_display:
        print("  [Chrome] Sin display Xvfb detectado, usando --headless")
        opts.add_argument("--headless")
    else:
        print(f"  [Chrome] Display Xvfb detectado: {os.environ.get('DISPLAY')}")
